...
static const int AVAILABLE = 1, FIRST = 2, SECOND = 4;

static volatile int usiserial_send_state = 1;


// Transmit data persistent between USI OVF interrupts
static volatile byte usiserial_tx_data;

static inline byte reverse_byte(byte x) {
    x = ((x >> 1) & 0x55) | ((x << 1) & 0xaa);
    x = ((x >> 2) & 0x33) | ((x << 2) & 0xcc);
    x = ((x >> 4) & 0x0f) | ((x << 4) & 0xf0);
    return x;
...
/**
 * Sends a byte. Blocks till the uart is available!!
 * @param data
 */
